...
class ReparamLargeKernelConv(nn.Module):  # 重参数化大核卷积操作

    def __init__(self, in_channels, out_channels, kernel_size,  # large size
                 stride, groups,
                 small_kernel,
...
class Block(nn.Module):
    ...
    def forward(self, x):
        input = x  # 32 6 32 896
        B, M, D, N = x.shape
        x = x.reshape(B, M * D, N)  # 128 192 896
        x = self.dw(x)  # 32 192 896
        x = x.reshape(B, M, D, N)
        x = x.reshape(B * M, D, N)  # 192 32 896
        x = self.norm(x)
        x = x.reshape(B, M, D, N)
        x = x.reshape(B, M * D, N)  # 32 192 896
        ...
        x1 = x.reshape(B, M, D, N)  # 32 6 32 896
...
class ModernTCN(nn.Module):   
    ...
    def __init__(self, task_name, patch_size, patch_stride, stem_ratio, downsample_ratio, ffn_ratio, num_blocks,
                large_size, small_size, dims, dw_dims,
                nvars, small_kernel_merged=False, backbone_dropout=0.1, head_dropout=0.1, use_multi_scale=False,
                revin=True, affine=True,
                subtract_last=False, freq=None, seq_len=512, c_in=7, individual=False, target_window=96, class_drop=0.,
                 class_num=None):
        ...
        # self.n_vars = c_in
        self.n_vars = c_in[0] if isinstance(c_in, (list, tuple)) else c_in
    ...
    def forward_feature(self, x, te=None):
        ...
        # B,M,L=x.shape #128 6 896
        x = x.unsqueeze(-2)  # 128 6 1 896

        for i in range(self.num_stage):
            B, M, D, N = x.shape  # 32 6 1 896
            x = x.reshape(B * M, D, N)  # 192 1 896
            if i == 0:
                ...
                if N % self.downsample_ratio != 0:
                    pad_len = self.downsample_ratio - (N % self.downsample_ratio)  # \\1
                    x = torch.cat([x, x[:, :, -pad_len:]], dim=-1)  # \\416 32 6\\416 64 4
            x = self.downsample_layers[i](x)  # 19232 896
            _, D_, N_ = x.shape  # 192 32 896
            x = x.reshape(B, M, D_, N_)  # 32 6 32 896
            x, x_k = self.stages[i](x)  # 32 13 32 5\\32 13 64 3\\32 13 128 2
        return x, x_k # 32 6 32 896

    def classification(self, x):

        # x = x.permute(0, 2, 1)
        # input x 32 6 896
        x, x_k = self.forward_feature(x, te=None)  #

        x_out = x

        x = self.act_class(x)  # 32 6 32 896
        x = self.class_dropout(x)  #
        x = x.reshape(x.shape[0], -1)  # 32 172032
        x = self.head_class(x)  # 32 2
        return x, x_out, x_k

    def forward(self, x, te=None):

        if self.task_name == 'classification':
            x, x_out, x_k = self.classification(x)  # 32 10
...
class base_Model(nn.Module):
    def __init__(self, configs,args):
        ...
        self.stem_ratio = args.stem_ratio
        self.downsample_ratio = args.downsample_ratio
        self.ffn_ratio = args.ffn_ratio
        self.drop_backbone = args.dropout
        self.drop_head = args.head_dropout
        ...
        self.model = ModernTCN(task_name=self.task_name, patch_size=self.patch_size, patch_stride=self.patch_stride,
        ...
        self.reducer = nn.Conv2d(self.nvars * configs.add, 128, kernel_size=1)

    def forward(self, x):
        # x = x.permute(0, 2, 1)  # 128 6 896
        # 统一输入为 [B, M(enc_in), L]
        if x.dim() == 3:
            # 若是 [B, L, M] 且 M=enc_in，则转置
            if x.shape[1] == self.seq_len and x.shape[2] == self.nvars:
                x = x.permute(0, 2, 1).contiguous()
        assert x.shape[1] == self.nvars, f"期望输入形状 [B,{self.nvars},L]，实际 {tuple(x.shape)}"

        te = None

        logits, x, x_k = self.model(x, te)
        B, M, D, N = x.shape
        x = x.reshape(B, M * D, 1, N)
        x = self.reducer(x)
        x = x.squeeze(2)

        x_k = x_k.reshape(B, M * D, 1, N)
        x_k = self.reducer(x_k)
        x_k = x_k.squeeze(2)

        return logits, x, x_k # 32 2  32,896,6