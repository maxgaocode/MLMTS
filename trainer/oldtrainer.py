...
# from sklearn.metrics import f1_score  # 导入 F1 分数
#
...
#         valid_loss, valid_acc, outs_valid, trgs_valid = model_evaluate(model, temporal_contr_model, valid_dl, device, training_mode)
#
#         if training_mode != 'self_supervised':  # 使用 scheduler 除了自监督模式
#             scheduler.step(valid_loss)
#
#         # 计算验证集的 F1 分数
#         if training_mode != 'self_supervised':
#             valid_f1 = f1_score(trgs_valid, outs_valid, average='macro')  # 或根据需要选择 'weighted', 'micro' 等
#             logger.debug(f'\nEpoch : {epoch}\n'
#                          f'Train Loss     : {train_loss:.4f}\t | \tTrain Accuracy     : {train_acc:2.4f}\n'
#                          f'Valid Loss     : {valid_loss:.4f}\t | \tValid Accuracy     : {valid_acc:2.4f}\n'
#                          f'Valid F1 Score : {valid_f1:.4f}')
#         else:
#             logger.debug(f'\nEpoch : {epoch}\n'
#                          f'Train Loss     : {train_loss:.4f}\t | \tTrain Accuracy     : {train_acc:2.4f}\n'
#                          f'Valid Loss     : {valid_loss:.4f}\t | \tValid Accuracy     : {valid_acc:2.4f}')
...
#
#     if training_mode != "self_supervised":  # 自监督模式无需在测试集上评估
#         # evaluate on the test set
#         logger.debug('\nEvaluate on the Test set:')
#         test_loss, test_acc, outs_test, trgs_test = model_evaluate(model, temporal_contr_model, test_dl, device, training_mode)
#         test_f1 = f1_score(trgs_test, outs_test, average='macro')
#         logger.debug(f'Test Loss      :{test_loss:0.4f}\t | Test Accuracy      : {test_acc:0.4f}\t | Test F1 Score      : {test_f1:.4f}')
#
#     logger.debug("\n################## Training is Done! #########################")
#
#
...
#         aug1, aug2 = aug1.float().to(device), aug2.float().to(device)#128.6.896
#
#         # optimizer
...
#                 outs.extend(pred.cpu().numpy().flatten())  # 扁平化并扩展列表
#                 trgs.extend(labels.data.cpu().numpy().flatten())  # 扁平化并扩展列表
#
#     if training_mode != "self_supervised":
...
def Trainer(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, train_dl, valid_dl, test_dl, device,
            logger, config, experiment_log_dir, training_mode, lambda1=1, lambda2=0.7, lambda3=0, results1=None,
            results_F1=None):
    ...
    for epoch in range(1, config.num_epoch + 1):
        train_loss, train_acc, train_loss_details, train_f1 = model_train(
            model, temporal_contr_model, model_optimizer, temp_cont_optimizer,
            criterion, train_dl, config, device, training_mode)

        valid_loss, valid_acc, valid_f1, _, _ = model_evaluate(
    ...
    if training_mode != "self_supervised":
        ...
        logger.debug(
            f'Test loss      :{test_loss:0.4f}\t | Test Accuracy      : {test_acc:0.4f}\t | Test F1 Score      : {test_f1:0.4f}')
...
def model_train(model, temporal_contr_model, model_optimizer, temp_cont_optimizer, criterion, train_loader, config,
    ...
    for batch_idx, (data, labels, aug1, aug2) in enumerate(train_loader):
        ...
        if training_mode == "self_supervised":
            ...
            temp_cont_loss1, temp_cont_lstm_feat1 = temporal_contr_model(features1, features2, x_k1)
            temp_cont_loss2, temp_cont_lstm_feat2 = temporal_contr_model(features2, features1, x_k2)
        ...
        if training_mode == "self_supervised":
            ...
            # 计算总损失，与GNN版本保持一致
            loss = (temp_cont_loss1 + temp_cont_loss2) * lambda1 + nt_xent_loss * lambda2
